...
// ✅ WASD · Mouse-look · Sit/Stand on chairs · Multiplayer · Background Music
...
const BOUNDS    = 11.5;
const WALK_SPD  = 4.5;
const SEAT_H    = 0.46; // height of chair seat above floor

// All chair positions in the scene: [x, z, facingAngle]
// facingAngle = which way the sitter faces (toward table center)
...
// Each table gets 4 chairs: right, left, front, back
// [offsetX, offsetZ, sitterFacingAngle]
const CHAIR_OFFSETS = [
  [ 0.78, 0,       Math.PI      ],   // right → face left (toward table)
  [-0.78, 0,       0            ],   // left  → face right
  [ 0,    0.78,   -Math.PI/2   ],   // front → face back
  [ 0,   -0.78,    Math.PI/2   ],   // back  → face front
];

// Build a flat list of all chairs with world positions
const ALL_CHAIRS = [];
TABLE_POSITIONS.forEach(([tx, tz], ti) => {
  CHAIR_OFFSETS.forEach(([ox, oz, faceAngle], ci) => {
    ALL_CHAIRS.push({
      ...
      pos:       [tx + ox, 0, tz + oz],
      faceAngle,
      chairRot:  faceAngle + Math.PI,   // chair mesh rotation
    });
  });
});
...
/* ─── Jazz tracks (free, public-domain style URLs) ───────── */
const TRACKS = [
...
/* ── Interactive Chair — glows when nearby, clickable ── */
...
/* ═══════════════════════════════════════════════════════════
   STAGE PERFORMER — permanent animated character on stage
   World position: stage is at [0,0,-10], platform top y=0.7
═══════════════════════════════════════════════════════════ */
...
/* Note particle that floats up from the performer */
...
/* The main performer — saxophone player with breathing, swaying, finger tapping */
function StagePerformer() {
  ...
  // Stage center position: stage at [0,0,-10], platform top y=0.7
  // Performer stands on it: world [0, 0.7, -10]
  ...
  useFrame((_, dt) => {
    ...
    // Saxophone sways with body naturally
    if (saxRef.current) {
      saxRef.current.rotation.z = -0.4 + Math.sin(time * 0.7) * 0.05;
    }

    // Spotlight flicker (subtle)
    if (spotRef.current) {
  ...
  // Note spawn positions (above performer's head)
  const noteOrigin = [STAGE_POS[0]+0.3, STAGE_POS[1]+2.4, STAGE_POS[2]];

  return (
    <group>
      {/* Animated spotlight above performer */}
      <spotLight
        ref={spotRef}
      ...
      <group ref={groupRef} position={STAGE_POS}>
        ...
        {/* Saxophone (held right side, angled up) */}
      ...
      {/* Name plate on stage floor in front of performer */}
...
/* ═══════════════════════════════════════════════════════════
   FULL CLUB SCENE — accepts chair interaction props
═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
   MY AVATAR  (sitting pose support)
═══════════════════════════════════════════════════════════ */
// FPP: camera IS the player — only a shadow disc is rendered for self
function MyAvatar({ posRef }) {
  ...
  return (
    <group ref={groupRef}>
      {/* Shadow disc only — body hidden in first-person */}
...
/* ═══════════════════════════════════════════════════════════
   REMOTE AVATAR (with sitting support)
═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
   PLAYER CONTROLLER — WASD + mouse-look + sit logic
═══════════════════════════════════════════════════════════ */
function PlayerController({ posRef, roomId, avatarType, onLocked, onSitFromKey, onToggleMic, onToggleMusic }) {
  ...
  useEffect(() => {
    const onChange = () => {
      ...
      onLocked(isLocked);
      // Explicitly restore cursor when pointer lock exits
      if (!isLocked) {
  ...
  useFrame((_, dt) => {
    ...
    if (!seated) {
      // WASD
      // FPP: W moves in the direction the camera faces (yaw)
      ...
      if (moving) {
        ...
        // rotY is set from yaw in the camera block below
      }
    } else {
      p.moving = false;
    }
    ...
    // ── First-Person Camera ──
    // Eye level: 1.55 standing, 0.95 seated
    const eyeY = seated ? SEAT_H + 0.52 : 1.55;

    // Camera sits exactly at player's eye position
    camera.position.set(p.x, eyeY, p.z);

    // Look direction driven by yaw (horizontal) + pitch (vertical)
    ...
    // Keep rotY in sync with yaw so body faces the right way
    p.rotY = yaw.current + Math.PI;

    // Throttled emit at 20Hz
...
/* ═══════════════════════════════════════════════════════════
   FLOATING EMOJI IN 3D SPACE — rises + fades above avatar
═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
   SPEAKING RING — pulsing ring above speaking avatars
═══════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════
   EMOJI REACTION BAR — bottom center
═══════════════════════════════════════════════════════════ */
...
function EmojiBar({ onEmoji, locked }) {
  ...
  // Close when pointer locks (entering FPP mode hides bar)
...
function MusicPlayer({ controlRef }) {
  ...
  // Expose togglePlay to parent via ref so Space key can trigger it
  useEffect(() => {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Vinyl icon — animated when playing */}
          <div style={{
...
export default function Room3D() {
  ...
  // posRef: mutable vector that WASD controller writes to each frame
  const myPosRef = useRef(Object.assign(
    new THREE.Vector3(...SPAWN),
    { rotY: Math.PI, moving: false, seated: false, chairId: null, nearbyChairId: null }
  ));

  const [players,      setPlayers]     = useState({});
  const [seatMap,      setSeatMap]     = useState({});   // chairId → socketId
  const [myChairId,    setMyChairId]   = useState(null);
  const [locked,       setLocked]      = useState(false);
  const [playerCount,  setPlayerCount] = useState(1);

  const [floatingEmojis, setFloatingEmojis] = useState([]); // [{id,emoji,x,z,ts}]
  const [micActive,      setMicActive]      = useState(false);
  const [speakingPlayers,setSpeakingPlayers]= useState({}); // socketId → bool
  ...
  // E key — interact with nearest chair
  ...
  useEffect(() => {
    ...
    socket.on("roomState", (state) => {
      ...
      // Rebuild seatMap from roomState
      const sm = {};
    ...
    socket.on("emojiReaction", ({ socketId, emoji }) => {
      const player = Object.entries(players).find(([sid]) => sid === socketId)?.[1];
  ...
  const toggleMic = useCallback(async () => {
    if (micActive) {
      ...
      try {
        ...
        // Voice activity detection — pulse speaking indicator
  ...
  return (
    <div
      style={{ width:"100vw", height:"100vh", background:"#080401", cursor: locked ? "none" : "auto" }}
    >
      <Canvas shadows camera={{ position:[0,1.55,6], fov:80 }}>
        ...
        {/* Permanent stage performer (showcase only) */}
        <StagePerformer/>

        <MyAvatar posRef={myPosRef}/>

        {Object.entries(players).map(([sid, d]) => (
          <RemoteAvatar key={sid} position={d.pos} color={d.color} seated={d.seated}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              width:1, height:14, background:"rgba(200,148,26,0.2)", margin:"0 4px",
            }}/>
        ...
        <div
          onClick={(e) => {
            e.stopPropagation();
            // Release pointer lock first so the browser doesn't block navigation
      ...
      {/* ── ENTRY OVERLAY (before pointer lock) ── */}
      {!locked && (
        <div
          style={{
        ...
        >
          <div style={{
            ...
            <p style={{fontSize:13, color:"#6b4a1a", marginBottom:24, lineHeight:1.6}}>
              Lock your mouse to look around<br/>and explore the Jazz Club in first-person
            </p>
            {/* Enter button */}
            <div
              onClick={requestLock}
            ...
            {/* Leave room link inside overlay too */}
            <div
      ...
      {/* ── BOTTOM CONTROL BAR (when locked) ── */}
      {locked && (
        <div style={{
...
const kbdStyle = {
  display:"inline-flex", alignItems:"center", justifyContent:"center",
  background:"rgba(200,148,26,0.1)", border:"1px solid rgba(200,148,26,0.25)",
  borderRadius:5, padding:"2px 7px", fontSize:10, color:"#c8941a",
  fontFamily:"system-ui", minWidth:20, fontWeight:700,
};
